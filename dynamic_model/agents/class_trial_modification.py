...
from supporting_files.bats_response_to_sound import (  # decide_next_direction_based_on_loudest_sound,
...
class Bat:
    ...
    def __init__(self, parameters_df, output_dir):
        ...
        self.allocentric_axis_y = Vector(1, np.sqrt(3)).normalize()  # self.direction
        ...
        self.ipi_matrix_for_consistency = given_parameters_df_return_grid_matrix_zeros(
            self.parameters_df
        )[0]
        self.memory_window_sum_matrix = self.ipi_matrix_for_consistency.copy()

        self.ipi_matrix_for_consistency_timeseries = []

        self.any_consistent_sound = "no"

        self.ipi_matrix_for_sensory_volume = generate_empty_matrix(self.parameters_df)[
            0
        ][0]
    ...
    def update_movement(self):
        ...
        # TODO: re-write this to incorporate oblique boundaries!
    ...
    def given_sound_objects_return_sounds_at_bat_position(
        ...
        for sound in sound_objects:
            # only active sounds can be heard by the bat
            is_sound_active = sound.active
            # self call shouldnt be heard by the bat
            is_sound_self_call = sound.emitter_id == self.id and isinstance(
                sound, DirectSound
            )
            # sounds generated by reflection on focal bat shouldn't also be heard by the bat
    ...
    def update_directon(self, current_time, sound_objects, temporal_masking_file):
        ...
        if (
            ...
            self.ipi_matrix_for_consistency = (
                given_parameters_df_return_grid_matrix_zeros(self.parameters_df)[0]
            )
            self.memory_window_sum_matrix = self.ipi_matrix_for_consistency.copy()
