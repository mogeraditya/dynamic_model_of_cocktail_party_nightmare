...
class Bat:
    ...
    def __init__(self, parameters_df, output_dir):
        ...
        self.time_since_last_cleanup = self.time_since_last_call # Clean up activates after every some steps to clear memory from RAM and store it on drive
    ...
    def _cleanup_sounds(self, current_time):
        ...
        if (current_time-self.time_since_last_cleanup)>=self.parameters_df["CLEANUP_INTERVAL"][0]: #10ms
            dir_to_store= self.output_dir+'/'+str(self.id)
            make_dir(dir_to_store)
            np.save(dir_to_store+f'/bat_{self.id}_received_sounds_snapshot_at_time_{"%.3f" % current_time}.npy', self.received_sounds)
            np.save(dir_to_store+f'/emitted_sounds_snapshot_at_time_{"%.3f" % current_time}.npy', self.emitted_sounds)
