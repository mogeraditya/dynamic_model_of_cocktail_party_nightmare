...
for INCLUDE_DIRECT_SOUNDS in INCLUDE_DIRECT_SOUNDS_ARR:
    for FOCAL_BAT in FOCAL_BATS:
        for AZIMUTH in AZIMUTH_THRESHOLDS:
            ...
            #     # layout="constrained",
            # )
            nan_matrix_direct_calls_theta = (
                np.zeros(shape=(len(received_sounds_sorted_by_time), NUM_COLORS - 1))
                * np.nan
            )
            nan_matrix_direct_calls_delta_t = (
                np.zeros(shape=(len(received_sounds_sorted_by_time), NUM_COLORS - 1))
                * np.nan
            )
            nan_matrix_self_echoes_theta = (
                np.zeros(
                    shape=(len(received_sounds_sorted_by_time), NUM_COLORS - 1 + 4)
                )
                * np.nan
            )
            nan_matrix_self_echoes_delta_t = (
                np.zeros(
                    shape=(len(received_sounds_sorted_by_time), NUM_COLORS - 1 + 4)
                )
                * np.nan
            ...
            for i, frame in enumerate(received_sounds_sorted_by_time):
                ...
                for sound_object in heard_sounds:
                    ...
                    if (
                        ...
                        direct_call_from = f"bat_{sound_object["emitter_id"]}"

                        if direct_call_from not in track_unique_direct_calls:
                            track_unique_direct_calls.append(direct_call_from)
                            times_of_sounds_to_plot.append([])
                            sounds_to_plot.append([])
                            delta_t.append([])
                        index = track_unique_direct_calls.index(direct_call_from)
                        ...
                        delta_t[index].append(d_time)
                        nan_matrix_direct_calls_theta[i, index] = theta
                        nan_matrix_direct_calls_delta_t[i, index] = d_time

                    if (
                        sound_object["type"] == "echo"
                        and sound_object["emitter_id"] == FOCAL_BAT
                    ):
                        theta = sound_object["bat_direction"].angle_between(
                            sound_object["incident_direction"]
                        ...
                        delta_t[index].append(d_time)
                        nan_matrix_self_echoes_theta[i, index] = theta
                        nan_matrix_self_echoes_delta_t[i, index] = d_time
                        # else:
                    #     color = "grey"
                    #     alpha = 0.5
                    #     axs.scatter(
            ...
            for i, nan_matrix in enumerate(nan_matrices):
                nan_matrices[i] = nan_matrices[i][:, ~np.isnan(nan_matrices[i]).all(0)]
                nan_matrices[i] = nan_matrices[i].T
                print(nan_matrices[i].shape)
                print(row_labels[i])
                nan_matrices[i] = nan_matrices[i][np.array(row_labels[i]).argsort()]
                row_labels[i] = np.array(row_labels[i])[
                    np.array(row_labels[i]).argsort()
                ]
            ...
            for i in range(0, 2):
                ...
                plt.subplot(2, 1, 1)
                plt.imshow(nan_matrices[i * 2], cmap=CMAP, aspect="equal")
                plt.title("Direct calls from other bats")
                plt.ylabel("direct call from")
                plt.xlabel("interpulse interval")
                plt.yticks(
                    labels=row_labels[i * 2],
                    ticks=np.arange(0, nan_matrices[i * 2].shape[0]),
                )
                if i < 1:
                    plt.colorbar(
                        location="bottom",
                        label="angle of incidence of sound",
                    )
                ...
                plt.subplot(2, 1, 2)
                plt.imshow(nan_matrices[i * 2 + 1], cmap=CMAP, aspect="equal")
                plt.title("Self echoes")
                plt.ylabel("self echo reflected from")
                plt.xlabel("interpulse interval")
                plt.yticks(
                    labels=row_labels[i * 2 + 1],
                    ticks=np.arange(0, nan_matrices[i * 2 + 1].shape[0]),
                )

                plt.suptitle(f"{NUM_COLORS} bats")
                ...
                # plt.show()
                dir_to_store = (
                    DIRECTORY_STORE_PLOTS + f"/self_echoes_vs_direct_calls/{FOCAL_BAT}/"
                )
                make_dir(dir_to_store)
                plt.tight_layout()
                plt.savefig(
                    dir_to_store
                    + f"bat_{FOCAL_BAT}consistency_of_{label}_with_angle_threshold_{np.round(np.degrees(AZIMUTH),1)}.png",
                    # transparent=True,
                )
                plt.clf()
