...
class DirectSound(EchoSound):
    def __init__(self, parameters_df, origin, creation_time, emitter_id):
        ...
        super().__init__(origin=origin, creation_time=creation_time, emitter_id=emitter_id, 
                         initial_spl=self.parameters_df['EMITTED_SPL'], parent_creation_time= self.parameters_df['EMITTED_SPL'], 
                         reflection_count=0)

        # self.initial_spl = Constants.EMITTED_SPL
        self.current_spl = self.initial_spl
        self.current_radius = 0
        self.max_radius = self.parameters_df['SOUND_DISK_WIDTH'] # Keep track of vada size
        self.active = True
    ...
    def create_echo(self, point, current_time, normal):
        ...
        reflected_spl = self.current_spl - self.parameters_df['REFLECTION_LOSS']
        if reflected_spl < self.parameters_df['MIN_DETECTABLE_SPL']:
            return None
